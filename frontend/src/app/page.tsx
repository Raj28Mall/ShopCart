          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-8">
            {(categories.slice(0,6)).map((category) => ( // to limit to 6
              <Link key={category.id} href={`/products?category=${category.slug}`}>
                <Card className="overflow-hidden transition-all hover:shadow-lg w-[30vw] pt-0 bg-orange-100">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 my-8 px-12">
            {(products.slice(0,8)).map((product) => (
              <Link key={product.id} href={`/products/${product.id}`}>
                <Card className="overflow-hidden transition-all hover:shadow-lg pt-0 bg-orange-100">
